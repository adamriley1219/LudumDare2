...
public:
	explicit DebugRenderObjectText2D( std::string text, eDebugRenderMode renderMode, Vec3 position = Vec3::ZERO, Vec2 pivot = Vec2::ZERO, Vec3 eulerRot = Vec3::ZERO, float textHeight = 1.0f,
		Rgba startColor = Rgba::WHITE, Rgba endColor = Rgba::WHITE, float lifeSpan = 0.0f, bool billboarded = true );
	virtual ~DebugRenderObjectText2D();
